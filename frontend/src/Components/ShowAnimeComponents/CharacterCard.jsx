...
export default function CharacterCard({character, voiceActor , withCharacterDetails}){
    return (
        ...
        { withCharacterDetails && <div className="flex md:!bg-none bg-center bg-cover w-[9rem] h-[12rem] md:w-auto md:h-auto" style={{ backgroundImage: `url(${character.character.images.jpg.image_url})` }}>
        ...
        <div className={`w-[9rem] h-[12rem]   md:h-full flex  md:ml-2 ${withCharacterDetails ? "md:w-auto" : "md:w-full"}`}>
            { voiceActor ?
                <>
                    <div className={`flex w-full items-start justify-end md:justify-start ${withCharacterDetails ? "md:items-end" : ""}  flex-col md:mr-2 md:mt-1 bg-center bg-cover md:!bg-none`} style={{ backgroundImage: `url(${voiceActor.person.images.jpg.image_url})` }}>
