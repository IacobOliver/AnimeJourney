...
export default function WriteReview({ closeEvent, handlePost }) {
    const [chars, setChars] = useState(0);
    const [user, setUser] = useAtom(state.user);
    const messageRef = useRef(null);

    return (
        <div  onClick={() => console.log("ok")} className="w-2/4 bg-[rgba(0,0,0,0.9)]  p-4">
            ...
            <div className="flex justify-between mb-5 mt-1">
                <div className="flex items-center">
                    <button onClick={() => {handlePost(messageRef), closeEvent()}} className="cursor-pointer font-semibold overflow-hidden relative border border-forth_color_theme group px-8 py-2 rounded-lg">
